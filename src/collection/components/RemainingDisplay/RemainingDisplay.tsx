...
const RemainingDisplay: React.FC<Props> = (props: Props) => {
  ...
  return (
    <div className="RemainingDisplayContainer">
      <div className="RemainingDisplayTitle">The monsters are having fun.</div>
      <div className="RemainingDisplayLabel">
        <img src={starIcon} />
        <div className="label">
          {isCollected ? (
            <>
              Reward Remaining Time:{" "}
              <div className="remain">{getRemain(remainMin)}</div>
            </>
          ) : (
